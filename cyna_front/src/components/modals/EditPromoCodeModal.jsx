...
import { updatePromoCode } from '../../api/promoCodes';

const EditPromoCodeModal = ({ isOpen, onClose, onSuccess, codeToEdit }) => {
  ...
  useEffect(() => {
    if (codeToEdit) {
      setForm({
        ...
        discountType: codeToEdit.discountType || 'percentage',
        discountValue: codeToEdit.discountValue || '',
        minOrderAmount: codeToEdit.minOrderAmount || '',
        maxDiscountAmount: codeToEdit.maxDiscountAmount || '',
        startDate: codeToEdit.startDate ? new Date(codeToEdit.startDate).toISOString().split('T')[0] : '',
        endDate: codeToEdit.endDate ? new Date(codeToEdit.endDate).toISOString().split('T')[0] : '',
        maxUses: codeToEdit.maxUses || '',
        isActive: codeToEdit.isActive ?? true
      });
    }
  }, [codeToEdit]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updatePromoCode(codeToEdit.id, dataToSend);
      onSuccess();
      onClose();
    } catch (err) {
      console.error('Erreur updating promo code:', err);
      alert(err.message || 'Une erreur est survenue lors de la modification du code promo');
    }
  };
